...
class Entrada
{
	...
	{
		$sql = "SELECT e.identrada,e.idusuario,u.nombre as usuario,u.cargo as cargo,u.cargo,lo.titulo as local, lo.imagen as local_imagen,t.titulo as tipo,p.nombre as proveedor,ubi.titulo as ubicacion,e.codigo,e.descripcion,e.descripcion,DATE_FORMAT(e.fecha_hora, '%d-%m-%Y %H:%i:%s') as fecha,e.total_compra,e.impuesto,e.estado,SUM(de.cantidad) as total_cantidad FROM entradas e LEFT JOIN locales lo ON e.idlocal=lo.idlocal LEFT JOIN ubicaciones ubi ON e.idubicacion=ubi.idubicacion LEFT JOIN tipos t ON e.idtipo=t.idtipo LEFT JOIN detalle_entrada de ON e.identrada = de.identrada LEFT JOIN proveedores p ON e.idproveedor=p.idproveedor LEFT JOIN usuario u ON e.idusuario=u.idusuario GROUP BY e.identrada ORDER BY e.identrada DESC";
		return ejecutarConsulta($sql);
	}
